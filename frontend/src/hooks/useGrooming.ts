...
export interface UseGroomingState {
  ...
  updateModerator: (userId: number) => Promise<void>;
 ...
 * ```
 */
export function useGrooming(roomId: number | undefined): UseGroomingState {
    ...
    async function init() {
      try {
        ...
        if (roomId) {
          ...
          if (storyId) {
            try {
              const storyVotes = await groomingService.getAllVotesForStory(storyId);
        ...
        setVotingFeedback({ type: "error", message: e.message ?? "Failed to initialize grooming" });
      }
    }
    init();
    return () => {
  ...
  const castVote = useCallback(async (size: number | null) => {
    if (!roomId || !currentUserId || !selectedStoryId) {
      setVotingFeedback({ type: "error", message: "Missing room, user, or story to vote." });
      return;
    }
    try {
      await groomingService.castVote(roomId, currentUserId, size);
      setVotingFeedback({ type: "success", message: "Vote submitted." });
      const refreshed = await groomingService.getAllVotesForStory(selectedStoryId);
      setVotes(refreshed);
    } catch (e: any) {
      setVotingFeedback({ type: "error", message: e.message ?? "Failed to vote." });
    }
  ...
  const revealVotes = useCallback(async () => {
    ...
    try {
      ...
      setVotingFeedback({ type: "error", message: e.message ?? "Failed to reveal votes." });
    }
  }, [roomId]);
  ...
  const submitSize = useCallback(async () => {
    ...
    try {
      ...
      setAverageSize(avg);
      setShowResult(true);
      setStories((prev) => prev.map((s) => (s.id === selectedStoryId ? { ...s, averageSize: avg } : s)));
    } catch (e: any) {
      setVotingFeedback({ type: "error", message: e.message ?? "Failed to submit size." });
    }
  }, [selectedStoryId]);
  ...
  const revote = useCallback(async () => {
    ...
    try {
      ...
      setVotes([]);
      setVotesRevealed(false);
      setVotingFeedback({ type: "success", message: "Votes cleared. Please revote." });
    } catch (e: any) {
      setVotingFeedback({ type: "error", message: e.message ?? "Failed to clear votes." });
    }
  }, [selectedStoryId]);

  const updateModerator = useCallback(async (userId: number) => {
    try {
      await groomingService.setModerator(userId);
      setModeratorId(userId);
      setPlayerFeedback({ type: "success", message: "Moderator updated successfully." });
    } catch (e: any) {
      setPlayerFeedback({ type: "error", message: e.message ?? "Failed to update moderator." });
    }
  }, []);
  ...
  const removeUser = useCallback(async (userId: number) => {
    try {
      ...
      setPlayerFeedback({ type: "error", message: e.message ?? "Failed to delete user." });
    }
  }, []);
