...
export const groomingService = {
  ...
  async getActiveUsers(roomId: number): Promise<Player[]> {
    const { status, result } = await Post("/user/active_users", { room_id: roomId });
    if (!status) throw new Error(result?.error ?? "Failed to load active users");
    return (result ?? []).map(toPlayer);
  },

  async addActiveUser(roomId: number, userId: number): Promise<void> {
    const { status, result } = await Post("/user/add_active_user", { room_id: roomId, user_id: userId });
  ...
  async getActiveStory(roomId: number): Promise<StoryId | null> {
    const { status, result } = await Get(`/room/${roomId}/active_story`);
    if (!status) return null;
    return result?.story_id ?? result?.storyId ?? null;
  },

  async setActiveStory(roomId: number, storyId: number): Promise<void> {
    const { status, result } = await Post("/room/set_story", { room_id: roomId, story_id: storyId });
    if (!status) throw new Error(result?.error ?? "Failed to set active story");
  },

  async castVote(roomId: number, userId: number, size: number | null): Promise<void> {
    const { status, result } = await Post("/user/vote", { room_id: roomId, user_id: userId, size });
    if (!status) throw new Error(result?.error ?? "Failed to vote");
  },

  async getVotesForRoom(roomId: number): Promise<Vote[]> {
    const { status, result } = await Post("/user/get_votes", { room_id: roomId });
    if (!status) throw new Error(result?.error ?? "Failed to get votes");
    return (result ?? []).map(toVote);
  },

  async getAllVotesForStory(storyId: number): Promise<Vote[]> {
    const { status, result } = await Post("/size/allvotes", { story_id: storyId });
    if (!status) throw new Error(result?.error ?? "Failed to load votes");
    return (result ?? []).map(toVote);
  },

  async revote(storyId: number): Promise<Story[]> {
    const { status, result } = await Post("/size/revote", { story_id: storyId });
    if (!status) throw new Error(result?.error ?? "Failed to revote");
    return (result ?? []).map(toStory);
  },

  async submitSize(storyId: number): Promise<string> {
    const { status, result } = await Post("/size/submit", { story_id: storyId });
    if (!status) throw new Error(result?.error ?? "Failed to submit size");
    return String(result);
  },

  async setModerator(userId: number): Promise<void> {
